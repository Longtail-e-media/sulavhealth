...
if (isset($_GET['page']) && $_GET['page'] == "product" && isset($_GET['mode']) && $_GET['mode'] == "subProductList"):
    ...
    <h3>
        List Sub Products of ["<?php echo Product::field_by_id($id, 'title'); ?>"]
        <a class="loadingbar-demo btn medium bg-blue-alt float-right" href="javascript:void(0);" onClick="addNewSubProduct(<?php echo $id; ?>);">
    ...
    <div class="example-box">
        <div class="example-code">
            <table cellpadding="0" cellspacing="0" border="0" class="table" id="subexample">
                ...
                <tbody>
                <?php $records = SubProduct::find_by_sql("SELECT * FROM " . $subpackageTablename . " WHERE type=" . $id . " ORDER BY sortorder DESC ");
...
<?php elseif (isset($_GET['mode']) && $_GET['mode'] == "addEditSubProduct"):
    ...
    <h3>
        ...
        <a class="loadingbar-demo btn medium bg-blue-alt float-right" href="javascript:void(0);"
           onClick="viewSubProductList(<?php echo $pid; ?>);">
            <span class="glyph-icon icon-separator"><i class="glyph-icon icon-arrow-circle-left"></i></span>
            <span class="button-content"> Back </span>
        </a>
    </h3>
    ...
    <div class="example-box">
        <div class="example-code">
            <form action="" class="col-md-12 center-margin" id="subproduct_frm">
                ...
                <input type="hidden" name="type" id="type" value="<?php echo !empty($subproductInfo->type) ? $subproductInfo->type : $pid; ?>"/>
                ...
                <div class="form-row">
                    ...
                    <div class="form-input col-md-4">
                        <select data-placeholder="Choose Field Type" class="chosen-selec validate[required,length[0,500]]" id="type" name="type">
                            <?php $categories = Product::find_by_sql("SELECT * FROM " . $moduleTablename . " WHERE type=$typeid ORDER BY sortorder DESC ");
                            if ($categories) {
                                foreach ($categories as $category) {
                                    $cat = !empty($subproductInfo->type) ? $subproductInfo->type : $pid;
