<script language="javascript">
    ...
    function create_editor1(base_url1, editor_arr1) {
        if (editor_arr1.length > 0) {
            for (var i in editor_arr1) {
                CKEDITOR.replace(editor_arr1[i], {filebrowserBrowseUrl: base_url1 + 'ckfinder/ckfinder.html'});
                /* For Read More*/
                var element = CKEDITOR.document.getById('readMore1');
                element.on('click', function (ev) {
                    var data = CKEDITOR.instances['content_greek'].getData();
                    if (data.match(/<hr id="system_readmore1" style="border-style: dashed; border-color: orange;" \/>/g)) {
                        showMessage('notice', 'Action already exists.');
                        return false;
                    } else {
                        CKEDITOR.instances['content_greek'].insertHtml('<hr id="system_readmore1" style="border-style: dashed; border-color: orange;" />');
                    }
                });
            }
        }
    ...
    $(function () {
        ...
        $(".character-keyword-gr").keyup(function () {
            var a = 250, b = $(this).val().length;
            if (b >= a) $(".keyword-remaining-gr").text(" you have reached the limit");
            else {
                var c = a - b;
                $(".keyword-remaining-gr").text(c + " characters left")
            }
        });

        $(".character-description-gr").keyup(function () {
            var a = 160, b = $(this).val().length;
            if (b >= a) $(".description-remaining-gr").text(" you have reached the limit");
            else {
                var c = a - b;
                $(".description-remaining-gr").text(c + " characters left")
            }
        });
    });
</script>