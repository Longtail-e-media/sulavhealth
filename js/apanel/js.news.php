<script language="javascript">
    ...
    $(document).ready(function () {
        ...
        $(".character-brief-gr").keyup(function () {
            var a = 250, b = $(this).val().length;
            if (b >= a) $(".brief-remaining-gr").text(" you have reached the limit");
            else {
                var c = a - b;
                $(".brief-remaining-gr").text(c + " characters left")
            }
        });
    });
    ...
    function deleteSavedNewsBannerimage(Re) {
        $('.MsgTitle').html('Do you want to delete the record ?');
        $('.pText').html('Clicking yes will be delete this record permanently. !!!');
        $('.divMessageBox').fadeIn();
        $('.MessageBoxContainer').fadeIn(1000);
        ...
        $(".botTempo").on("click", function () {
            var popAct = $(this).attr("id");
            if (popAct == 'yes') {
                $('#removeSavedBannerimg' + Re).fadeOut(1000, function () {
                    $('#removeSavedBannerimg' + Re).remove();
                    $('.uploaderB').fadeIn(500);
                });
            } else {
                Re = '';
            }
