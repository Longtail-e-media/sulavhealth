	...
	switch($action)
	{
        ...
        case "request_inquiry":
            ...
            if ($bookRec->save()) {
                ...
                // actual payment
                if (!empty($simplifyToken)) {
                    $bookRecord = BookingInfo::find_by_id($booking_id);
                    ...
                    $paymentPayload = array(
                        'reference' => $bookRecord->accesskey,
                        'amount' => $amount,
                        'token' => $simplifyToken,
                        'description' => 'Evonymon Payment - ' . $bookRecord->accesskey,
                        'currency' => 'EUR'
                    );
                    $response = array();
                    ...
                    try {
                        ...
                        if ($payment->paymentStatus == 'APPROVED') {
                            ...
                            SET 
                                ...
                                cc_id = '" . $payment->card->id . "',
                                cc_type = '" . $payment->card->type . "',
                        ...
                        $response["error"] = $e->getMessage();
                        echo json_encode(array("action" => "unsuccess_payment", "message" => $response));
                        exit;
                    }
                }
                ...
                $message = ($lang == "gr")
                    ? 'Επιτυχής Υποβολή! Η παραγγελία σας καταχωρήθηκε με επιτυχία.' . $newAccountMsg
