<?php 
	...
	{						
        ...
        case "addFrontSubscriber":
            foreach ($_REQUEST as $key=>$val){$$key=$val;}
            $record = new Subscribers();

            $record->title			= $email;
            $record->mailaddress	= $email;
            $record->status 		= 1;
            $record->sortorder		= Subscribers::find_maximum();
            $record->added_date		= registered();

            $chkRec = Subscribers::checkDupliEmail($email);
            if($chkRec):
                echo json_encode(array("action" => "warning", "message" => "Email already registered !!!"));
                exit;
            endif;

            $db->begin();
            if ($record->save()): $db->commit();
                echo json_encode(array("action" => "success", "message" => "Your registration has been successfully submitted. Thank you for subscription."));
                log_action("Subscriber " . $email . " added Successfully", 1, 3);
            else:
                $db->rollback();
