...
class Package extends DatabaseObject
{
    ...
    protected static $db_fields = array('id', 'slug', 'image', 'header_image', 'banner_image', 'title', 'detail', 'status', 'sortorder', 'sub_title', 'content', 'procedures', 'test_preparation', 'title_greek', 'content_greek', 'brief_greek', 'sub_title_greek', 'procedure_greek', 'test_preparation_greek', 'meta_title_greek', 'meta_keywords_greek', 'meta_description_greek', 'meta_title', 'meta_keywords', 'meta_description', 'homepage', 'added_date', 'modified_date', 'list_image');
    ...
    var $sub_title_greek;
    var $procedure_greek;
    var $test_preparation_greek;
    var $procedures;
    var $test_preparation;
    var $meta_title;
    var $meta_keywords;
    var $meta_description;
    var $meta_title_greek;
    var $meta_keywords_greek;
    var $meta_description_greek;
    var $homepage;
    var $added_date;
    var $modified_date;
    var $list_image;
    ...
    {
        global $db;
        $sql = "SELECT id FROM " . self::$table_name . " WHERE status='1' AND homepage='1' ORDER BY sortorder DESC LIMIT 1 ";
        $result = $db->query($sql);
    ...
    static function getHomePackage($limit = '')
    {
        global $db;
        $cond = !empty($limit) ? ' LIMIT ' . $limit : '';
        $sql = "SELECT * FROM " . self::$table_name . " WHERE status=1 AND homepage=1 ORDER BY sortorder DESC $cond";
        return self::find_by_sql($sql);
    }

    // view package Front.
    static function getPackageByHomepage($homepage, $limit = '')
    {
        global $db;
        $cond = !empty($limit) ? ' LIMIT ' . $limit : '';
        $sql = "SELECT * FROM " . self::$table_name . " WHERE status=1 AND homepage != $homepage ORDER BY sortorder DESC $cond";
        return self::find_by_sql($sql);
    }
    ...
    /************************** Package link  by me ***************************/
    public static function get_internal_link($Lsel = '', $LHomepage = 0)
    {
        global $db;
        $sql = "SELECT id, slug, title, homepage FROM " . self::$table_name . " WHERE status='1' ORDER BY sortorder ASC";
...
//				$chkChild  = Subpackage::getTotalSub($pageRow->type);
    ...
    public static function homepage_by_id($homepage = 1)
    {
        global $db;
        $sql = "SELECT id FROM " . self::$table_name . " WHERE homepage={$homepage} LIMIT 1";
        $result = $db->query($sql);
        $return = $db->fetch_array($result);
        return ($return) ? $return['id'] : 0;
    }
