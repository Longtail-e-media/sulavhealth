...
class Member extends DatabaseObject
{
    ...
    protected static $db_fields = array(
        'id', 'name', 'phone', 'email', 'password', 'accesskey', 'image', 'access_code', 'sortorder', 'status', 'created_at', 'updated_at');
    ...
    public static function get_access_by($token = '')
    {
        ...
        $sql = "SELECT id, name, email, accesskey FROM " . self::$table_name . $cond;
        $result = self::find_by_sql($sql);
        return !empty($result) ? array_shift($result) : false;
    }
    ...
    {
        global $db;
        $result_array = self::find_by_sql("SELECT `id`,`email`,`name` FROM " . self::$table_name . " WHERE `email`='" . $email . "' LIMIT 1");
    ...
    {
        global $db;
        $sql = "SELECT id, name, email FROM " . self::$table_name . " WHERE id=" . $id;
        $result = self::find_by_sql($sql);
        return !empty($result) ? array_shift($result) : false;
    }

    // get the externaluser name from it's id
    static function getMemberName($id = 0)
    {
        global $db;
        $result = $db->query("SELECT name FROM " . self::$table_name . " WHERE id='{$id}'");
        $return = $db->fetch_array($result);
        return ($return) ? $return['name'] : '';
    }
