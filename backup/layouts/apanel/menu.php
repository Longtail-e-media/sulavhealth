...
<ul>
    ...
    $unansweredQuestionCount = Userquestion::get_total_unanswered_question_per_user();
    ...
    foreach ($parentmenu as $key=>$val){
        ...
        if(!empty($page)):
            ...
            $style = (($unansweredQuestionCount > 0) and ($val->id == 18)) ? 'color:red' : '';
        echo '<li class="'.$currpage.$actvpage.'">
                <a href="'.$pagelink.'" title="'.$val->name.'" style="'.$style.'">
