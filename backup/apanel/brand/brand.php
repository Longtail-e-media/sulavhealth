...
<?php
$moduleTablename = "tbl_brands"; // Database table name
$moduleId = 26;                // module id >>>>> tbl_modules
$moduleFoldername = "brand";        // Image folder name

if (isset($_GET['page']) && $_GET['page'] == "brand" && isset($_GET['mode']) && $_GET['mode'] == "list"):
    ...
    <div class="example-box">
        <div class="example-code">
            <table cellpadding="0" cellspacing="0" border="0" class="table" id="example">
                ...
                <tbody>
                <?php $records = Brand::find_by_sql("SELECT * FROM " . $moduleTablename . " ORDER BY sortorder DESC ");
...
<?php elseif (isset($_GET['mode']) && $_GET['mode'] == "addEdit"):
    if (isset($_GET['id']) && !empty($_GET['id'])):
        $brandId = addslashes($_REQUEST['id']);
        $brandInfo = Brand::find_by_id($brandId);
        $status = ($brandInfo->status == 1) ? "checked" : " ";
        $unstatus = ($brandInfo->status == 0) ? "checked" : " ";
        $homepage = ($brandInfo->homepage == 1) ? "checked" : " ";
        $nothomepage = ($brandInfo->homepage == 0) ? "checked" : " ";
    endif;
    ?>
    <h3>
        <?php echo (isset($_GET['id'])) ? 'Edit Brand' : 'Add Brand'; ?>
        <a class="loadingbar-demo btn medium bg-blue-alt float-right" href="javascript:void(0);"
           onClick="viewBrandList();">
            ...
            <form action="" class="col-md-12 center-margin" id="articles_frm">
                <div class="form-row">
                    ...
                    <div class="form-input col-md-20">
                        <input placeholder="Title" class="col-md-6 validate[required,length[0,200]]" type="text"
                               name="title" id="title"
                               value="<?php echo !empty($brandInfo->title) ? $brandInfo->title : ""; ?>">
                    </div>
                </div>
                ...
                <div class="form-row add-image">
                    ...
                    if (!empty($brandInfo->image)) {
                        $imgRec = unserialize($brandInfo->image);
                        if (is_array($imgRec)) {
                            foreach ($imgRec as $key => $recimg) {
                                $deleteid = rand(0, 99999);
                                $imagePath = SITE_ROOT . 'images/brand/' . $recimg;
                                        ...
                                        <div class="infobox info-bg">
                                            <div class="button-group" data-toggle="buttons">
                                                <span class="float-left">
                                                    <?php
                                                    if (file_exists(SITE_ROOT . "images/brand/" . $recimg)):
                                                        $filesize = filesize(SITE_ROOT . "images/brand/" . $recimg);
                                            ...
                                            <img src="<?php echo IMAGE_PATH . 'brand/thumbnails/' . $recimg; ?>"
                                                 style="width:100%"/>
                ...
                <input myaction='0' type="hidden" name="idValue" id="idValue" value="<?php echo !empty($brandInfo->id) ? $brandInfo->id : 0; ?>"/>
            </form>
        </div>
    </div>
    <script>
        ...
        $(document).ready(function () {
            $('#gallery_upload').uploadify({
                ...
                'onUploadSuccess': function (file, data, response) {
                    ...
                    var filename = data;
                    $.post('<?php echo BASE_URL;?>apanel/brand/uploaded_image.php', {imagefile: filename}, function (msg) {
