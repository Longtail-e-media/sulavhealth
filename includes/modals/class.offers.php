...
class Offers extends DatabaseObject
{
    ...
    public static function get_internal_link_test($Lsel = '', $LType = 0)
    {
        global $db;
        $sql = "SELECT slug, title FROM " . self::$table_name . " WHERE status='1' ORDER BY sortorder ASC";
        $pages = Offers::find_by_sql($sql);
        ...
        if ($pages):
            ...
            foreach ($pages as $pageRow):
                ...
                $result = '<option value="' . $pageRow->slug . '" ' . $sel . '>&nbsp;&nbsp;Offers</option>';
            endforeach;
            $result .= '</optgroup>';
        endif;
        return $result;
    ...
    public static function find_allactive_offers()
    {
        global $db;
        return self::find_by_sql("SELECT * FROM " . self::$table_name . " WHERE status=1 ORDER BY sortorder ASC");
    }
    ...
    static function find_by_home()
    {
        global $db;
        $result_array = self::find_by_sql("SELECT * FROM " . self::$table_name . " WHERE status=1 ORDER BY sortorder ASC LIMIT 1");
        return !empty($result_array) ? array_shift($result_array) : false;
    }
