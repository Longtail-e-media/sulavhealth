...
class News extends DatabaseObject
{
    ...
    protected static $db_fields = array(
        'id', 'cat_id', 'slug', 'title', 'author', 'brief', 'content', 'title_greek', 'content_greek', 'brief_greek', 'status', 'meta_keywords', 'meta_description',
        'meta_title_greek', 'meta_keywords_greek', 'meta_description_greek', 'news_date', 'archive_date', 'sortorder', 'type', 'viewcount', 'added_date', 'modified_date',
        'image', 'banner_image', 'source'
    );
    ...
    public $meta_title_greek;
    public $meta_keywords_greek;
    public $meta_description_greek;
    public $news_date;
    public $archive_date;
    public $sortorder;
    public $type;
    ...
    // view package Front.
    static function getNewsByTypeNews($limit = '')
    {
        global $db;
        $cond = !empty($limit) ? ' LIMIT ' . $limit : '';
    ...
    // view package Front.
    static function getNewsByTypeSeminar($limit = '')
    {
        global $db;
        $cond = !empty($limit) ? ' LIMIT ' . $limit : '';
    ...
    // GET NEWS LIST.
    public static function getNewsList($total = 50)
    {
        global $db;
    ...
    // get  other news
    public static function get_other_news($notid = '', $limit = '')
    {
        global $db;
        $cond2 = !empty($notid) ? ' AND id<>' . $notid : '';
        $cond3 = !empty($limit) ? ' LIMIT ' . $limit : '';
        $sql = "SELECT * FROM " . self::$table_name . " WHERE status=1  $cond2 ORDER BY news_date DESC " . $cond3;
        return self::find_by_sql($sql);
    }
}
